...
#### Amazon S3
If `Amazon S3` should be enabled as `Default File Storage` it is necessary to set the environment variable or just not to specify it.

```bash
DJANGO_USE_LOCAL_FILE_STORAGE=no
```

In addition, you should specify variables related to the functioning of `Amazon S3`.
```bash
...
You can find values of these variables in project's deployment details document.
